...
Fires GitHub `workflow_dispatch` at **09:15 ET** (premarket) and **16:30 ET** (overnight) on weekdays. Does not call Finviz. Does not send `force`.
...
Cron is `*/15 * * * *`. The handler no-ops unless the America/New_York wall clock is exactly 09:15 or 16:30 on a weekday.

Spare clock: [cron-job.org](https://cron-job.org) POSTing the same GitHub dispatch URL with that PAT.

Manual: Actions → premarket or overnight → Run workflow → `force` if you missed the window.
