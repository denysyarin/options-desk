...
Fires GitHub `workflow_dispatch` at **09:00 ET** (premarket prelayer), **09:30 ET** (RTH live chains), and **16:30 ET** (overnight) on weekdays. Does not call Finviz. Does not send `force`.
...
Cron is `*/15 * * * *`. The handler no-ops unless the America/New_York wall clock is exactly 09:00, 09:30, or 16:30 on a weekday.
